...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    guests.each_with_index do |guest, index|
        ...
        config.vm.define guest[:vagrant_name] do |machine|
            ...
            # Replace --compress from the vagrant default with --new-compress
            # for compatibility
            default_rsync_args = [
                "--verbose",
                "--archive",
            ...
            if ansible_mode == 'guest' and guest[:name] == 'provisioner'
                ...
                machine.vm.synced_folder ".", "/#{role_name}",
                    type: "rsync",
                    rsync__args: default_rsync_args
                machine.vm.synced_folder "./.vagrant/machines", "/machines",
                    type: "rsync",
                    rsync__args: default_rsync_args + ["--chmod=D700,F600"]
            else
                machine.vm.synced_folder '.', '/vagrant',
                    type: "rsync",
                    rsync__args: default_rsync_args
